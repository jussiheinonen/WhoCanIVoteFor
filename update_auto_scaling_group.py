...
def _filter_by_code_deploy_tag(asg):
    """
    Checks if an auto scaling group has CodeDeploy tag
    """
    for tag in asg["Tags"]:
...
def get_latest_code_deploy_tagged_asg():
    """
    Gets all auto scaling groups, filters them to only those with the
...
def update_asg(asg):
    """
    Updates the number of instances in an auto scaling group
    """
...
def check_asg_instances_healthy(asg):
    """
    Checks the health and lifecycle states of instances in an auto
    scaling group.
    """
